...
class ShowCollectionList(
    private val itemsList: ArrayList<collectioncustomermodel>,
    private val listener: AdapterClickListener
) : RecyclerView.Adapter<ShowCollectionList.MyViewHolder>() {
...
val custname=itemsList[position].getCustomerName()
      ...
      val mobile_no=itemsList[position].getMobileNo()
        println("mobileno adapter //$mobile_no")
        holder.txt_collected.text =itemsList[position].getMobileNo()

        //  holder.txt_collected.text= Constants.money_convertor(
       //     Constants.isEmtytoZero(itemsList[position].getTotalPaid()!!),false)
      //  holder.txt_tobecollected.text= Constants.money_convertor(Constants.isEmtytoZero(itemsList[position].getTotalPending()!!),
       //     false)
        val cus_code=itemsList[position].getCustomerCode()
        println("cuscode adapter //$cus_code")
        holder.txt_tobecollected.text =itemsList[position].getCustomerCode()
    }
